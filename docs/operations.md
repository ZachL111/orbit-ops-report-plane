...
`orbit-ops-report-plane` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for report scenarios with bounded scenario files, conflict explanations, and bounded memory input sets.
...
The score is deterministic. The current threshold is 157. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 2.
